...
- **RFH-TC-022** (tests: RFH-AC-022): Steps: replay a recorded corpus of atypical-input sessions (switch access, dictation cadence, non-native keyboards) through the scoring pipeline against per-account baselines built from at least 10 prior posts each, alongside an equal-size typical-input control corpus. Expected: block and suspect rates for the atypical corpus fall within the control corpus rates plus normal sampling variance.
