...
- **RFH-NFR-006** (traces: RFH-PR-018): The collector shall feature-detect its APIs per browser rather than parse user agents: on desktop browsers all four signal classes shall be captured, and where pointer APIs are absent (mobile browsers) mouse signals shall be omitted without raising errors, with the remaining classes still captured and the omission recorded in the payload for score normalization.
